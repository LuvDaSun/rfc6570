...
function UriTemplate(template){
	...
	this.parse = function (str){
		...
		if(!parts.every(function (part, partIndex) {
			data[part[1]] = unescape(
				partIndex + 1 < offsets.length - 1
				? str.substring(offsets[partIndex], offsets[partIndex + 1] - 1)
				: str.substring(offsets[partIndex])
			);
			return true;
		})) return false;
